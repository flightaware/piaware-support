node(label: 'raspberrypi') {
    ...
    for (int i = 0; i < dists.size(); ++i) {
        ...
        stage("Test install on ${dist}") {
            sh "/build/repo/validate-packages.sh ${dist} ${results}/piaware-support_*.deb ${results}/piaware-repository_*.deb ${results}/piaware-repository-testing_*.deb ${results}/piaware-release_*.deb"
        }
    }
