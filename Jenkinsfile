node(label: 'raspberrypi') {
    properties([
        pipelineTriggers([
            upstream(threshold: 'SUCCESS',
                     upstreamProjects: "piaware/${env.BRANCH_NAME}, beast-splitter/${env.BRANCH_NAME}, dump1090/${env.BRANCH_NAME}, dump978/${env.BRANCH_NAME}, piaware-web/${env.BRANCH_NAME}, piaware-ble-connect/${env.BRANCH_NAME}, piaware-configurator/${env.BRANCH_NAME}, flightaware-apt-repository/${env.BRANCH_NAME}")
        ]),
    ...
    for (int i = 0; i < dists.size(); ++i) {
        ...
        stage("Test install on ${dist}") {
            sh "BRANCH=${env.BRANCH_NAME} /build/pi-builder/scripts/validate-packages.sh ${dist} ${results}/piaware-support_*.deb ${results}/piaware-release_*.deb"
        }
    }
